...
One `--mcp-config` (claude-format JSON, `{"mcpServers":{…}}`) now reaches every backend that can take it, and the
OpenAI shim routes by **model name** (gpt\*/codex\*/sol/terra/luna → codex, `copilot-*` → copilot, `<backend>:<model>`
pins, everything else → claude) instead of hardwiring claude:
