...
func cmdServe(args []string) error {
	...
	openaiHomeRoot := fs.String("openai-home-root", "", "dir holding role-specific seat environments, selected by a \"<model>#<role>\" model name: <root>/<role>-claude-home (mounted as an isolated claude seat's ~/.claude), <root>/<role>-workdir (the seat's grounding context; claude mounts it ro as /work, host-run seats cd into it), and <root>/<role>-skills (authored skills; claude seats read them via the home's skills/, host-run codex/copilot/opencode seats get them mirrored into the role workdir at open). Each piece resolves independently; a missing dir degrades, never fails.")
